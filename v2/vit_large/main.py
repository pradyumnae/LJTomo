...
def main(cfg: DictConfig):
    # Initialize DDP using rendezvous or environment parameters
    ...
    if global_rank == 0:
        run_name = "LeJEPA_Tomography_ViT_Large_DINO" if cfg.pretrained else "LeJEPA_Tomography_ViT_Large_Scratch"
        wandb.init(project="LeJEPA_Tomography_Large", name=run_name, mode="disabled" if cfg.debug else "online", config=dict(cfg))
    ...
    probe = nn.Sequential(nn.LayerNorm(1024), nn.Linear(1024, 10)).to("cuda")
...
if __name__ == "__main__":
    ...
    default_cfg = OmegaConf.create({
        "debug": True,
        "pretrained": True, # Pre-trained DINOv2 vs Scratch
        "dataset_key": None,
