...
def generate_pca_viz(model, dataset, num_samples=5, seed=42):
    ...
    with torch.no_grad():
        for i, (img, _) in enumerate(loader):
            ...
            # Dinov2 vit_large has 1 CLS token + 4 register tokens (total 5)
            num_prefix = backbone.num_prefix_tokens if hasattr(backbone, 'num_prefix_tokens') else 5
            ...
            pcs = patch_tokens @ v[:, :3]
            pcs = pcs.reshape(grid_size, grid_size, 3).permute(2, 0, 1).unsqueeze(0)
            
            # Upsample to 512x512
            pca_img = F.interpolate(pcs, size=(512, 512), mode='bilinear', align_corners=False)
            pca_img = pca_img.squeeze(0).permute(1, 2, 0).cpu().numpy()
...
if __name__ == "__main__":
    ...
    print("Initializing DINOv2 ViT-Large model...", flush=True)
